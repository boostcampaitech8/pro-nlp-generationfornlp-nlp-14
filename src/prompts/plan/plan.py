...
# 역할
문제를 분석하여 외부 지식 검색이 필요한지 판단하고, 필요시 효과적인 검색 쿼리를 생성한다.

# 출력 형식
- 반드시 JSON만 출력 (설명/마크다운/코드블록 금지)
- 스키마: {{"requests": [{{"query": string, "top_k": integer}}]}}
...
PLANNER_USER = """아래 문제를 분석하여 검색 쿼리를 생성하라.
...
작업:
1. 지문/선택지만으로 답을 찾을 수 있는지 판단
2. 외부 지식이 필요하면 1~3개의 검색 쿼리 생성
3. 각 쿼리는 고유명사 포함, 60자 이내, placeholder 금지
4. 조건을 만족하는 쿼리를 만들 수 없으면 requests=[]
